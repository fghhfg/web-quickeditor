...
function on_gapi_loaded(){
  window.init_gapi ? window.init_gapi() : setTimeout(on_gapi_loaded,10)
}
...
module.controller(
  "RenameCtrl", 
  [
    "$scope", "$mdDialog", "title", 
    function (e,t,n) { 
      e.form = {title:n}, 
      this.save = function () { t.hide(e.form.title) }, 
      this.cancel = function(){t.cancel()}
    }
  ]
),

angular.module("editor.rename").service(
  "renameDialog", 
  [
    "$mdDialog", 
    function(e) {
...
module.controller(
  "MainCtrl", 
  [
    "$scope","$location","$routeParams","$q","$mdToast","drive","login","renameDialog",
    function(e,t,n,i,o,l,r,a){
      var d = {content:"",metadata:{id:null,title:"untitled.txt",mimeType:"text/plain",editable:!0}};
      e.file = null, e.loading =! 0;
      var c = function(e){o.show(o.simple().content(e))}, 
        u = 
          function(){
            return 
              l.saveFile(e.file.metadata,e.file.content)
                .then(
                  function(t){
                    return m(t.metadata.id), e.file = t, c("File saved"), e.file
                  },
                  function(e){return c("Unable to save file"), i.reject(e)}
                )
          }, 
        s = 
          function(t){
            var n = t ? l.loadFile(t) : i.when(d);
            return 
              n.then(
                function(t){return e.file = t, e.file}, 
                function(){return t && c("Unable to load file"), s()}
              )
          },
        m = function(n){ e.file.metadata.id != n && (t.path("/edit/"+n), t.search(""), t.replace()) };
        this.saveFile = function(t){return null===e.file.metadata.id?this.renameFile(t):u()},
        this.renameFile = 
          function(t){
            return a.show(t,e.file.metadata.title).then(function(t){return e.file.metadata.title = t, u()})},
        this.openFile = function(){l.showPicker().then(function(e){m(e)})},
        this.shareFile = 
          function(t){
            null === 
              e.file.metadata.id ? 
              e.ctrl.renameFile(t).then(function(){l.showSharing(e.file.metadata.id)}) : 
              l.showSharing(e.file.metadata.id)};
      var p = angular.bind(e.ctrl,s,n.fileId);
      r.checkAuth(n.user)
